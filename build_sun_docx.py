...
def main():
    ...
    pieces = [
        ('10 余年外资制造业物流关务经验', True),
        ('，深耕世界 500 强德资企业及大型显示面板龙头，精通进出口物流（海/陆/空运）、关务申报与合规、AEO 体系建设、订单与供应商管理。', False),
        ('德语专业八级（PGH）', True),
        ('，', False),
        ('英语六级且可作为工作语言', True),
        ('，能独立对接海外总部、海关与外籍客户。熟悉 SAP 主数据、加工贸易、保税监管、出口退税及鼓励类减免税政策，曾主导多项流程优化和成本节约项目，', False),
        ('具备跨部门协同与海关稽查应对的实战经验', True),
        ('。', False),
    ]
    for text, bold in pieces:
        add_run(intro, text, bold=bold, size=10, color=NAVY if bold else TEXT_DARK)
    ...
    add_run(summary, '负责京东方青岛工厂设备/备件/原材料/消耗品的', size=10, color=TEXT_DARK_GREY)
    add_run(summary, '进出口物流与通关全流程', bold=True, size=10, color=NAVY)
    add_run(summary, '，统筹关务合规、AEO 持续符合管理、海关对接及跨部门审核支持。',
    ...
    add_run(summary2, '独立负责公司', size=10, color=TEXT_DARK_GREY)
    add_run(summary2, '进出口物流与关务全链路', bold=True, size=10, color=NAVY)
    add_run(summary2, '——从供应商管理、订单运输、通关申报到政策应用、AEO 体系与单证存档；服务期内多次实现',
            size=10, color=TEXT_DARK_GREY)
    add_run(summary2, '关税减免与物流降本', bold=True, size=10, color=NAVY)
    add_run(summary2, '。', size=10, color=TEXT_DARK_GREY)

    sub_title(doc, '物 流 与 供 应 商')
    bullet(doc, '供应商管理', '筛选报关行与第三方物流商，议定海/空/铁运价并定期评估，降低报关差错率、提升送达及时率。',
           sub_bold_keywords=['降低报关差错率、提升送达及时率'])
    ...
    set_paragraph_spacing(p_pl, before=0, after=1, line=1.4)
    p_pl.paragraph_format.left_indent = Cm(0.4)
    p_pl.paragraph_format.first_line_indent = Cm(-0.4)
    add_run(p_pl, '▸ ', bold=True, size=10, color=NAVY)
    add_run(p_pl, '政策红利', bold=True, size=10, color=NAVY)
    add_run(p_pl, ' ｜ ', size=10, color=TEXT_GREY)
    add_run(p_pl, '★ 成功为公司申请鼓励类产业项目确认书，实现设备进口关税减免，显著节省成本。',
    ...
    set_paragraph_spacing(p_de, before=0, after=1, line=1.4)
    p_de.paragraph_format.left_indent = Cm(0.4)
    p_de.paragraph_format.first_line_indent = Cm(-0.4)
    add_run(p_de, '▸ ', bold=True, size=10, color=NAVY)
    add_run(p_de, '德语支持', bold=True, size=10, color=NAVY)
    add_run(p_de, ' ｜ ', size=10, color=TEXT_GREY)
    add_run(p_de, '陪同接待来访德国客户，承担德语类文件翻译', bold=True, size=10, color=NAVY)
    add_run(p_de, '，为商务谈判提供语言支持。', size=10, color=TEXT_DARK)
    ...
    for label, parts in rows:
        ...
        for text, bold in parts:
            add_run(sp, text, bold=bold, size=10, color=NAVY if bold else TEXT_DARK)

    out = '/Users/tiny/Desktop/resume/孙晓燕_物流关务简历_含证件照.docx'
    doc.save(out)
    print(f'saved -> {out}')
